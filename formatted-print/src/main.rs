...
fn main() {
    /* Printing in rust is handled by a series of macros that are defined in the standard library (std::fmt)
     *
    ...
    // Special formatting
    println!(
        "{} in {:b} people know binary, the other half doesn't",
        1, 2
    );
    ...
    println!(
        "{0}.{1}.{2}.{3} -> {0:08b}.{1:08b}.{2:08b}.{3:08b}",
        192, 168, 1, 2
    );
